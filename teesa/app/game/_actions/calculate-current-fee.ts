...
export async function calculateCurrentFee(): Promise<bigint> {
    ...
    // Calculate the current fee: initialFee * (1.1 ^ questionCount)
    if (questions.length === 0) {
        return initialFee;
    }
    
    // Calculate 1.1^questionCount using BigInt math
    ...
    const BASE_MULTIPLIER = BigInt(110); // 1.1 as 110/100
    const PRECISION = BigInt(100);
    
    let multiplier = PRECISION; // Start with 1.0 (100/100)
    
    for (let i = 0; i < questions.length; i++) {
        // Multiply by 1.1 each time (110/100)
